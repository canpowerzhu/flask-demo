...
def get_sys_config_list(*queries) -> object:
    """
   ...
   """
    ...
    for query in queries:
        query_filter.append(
            or_(
                SysConfigInfo.config_name.ilike(f'%{query}%'),
                SysConfigInfo.config_key.ilike(f'%{query}%'),
                SysConfigInfo.config_group.ilike(f'%{query}%')
            )
        )

    try:

        result = SysConfigInfo.query.all() if len(query_filter) == 0 else SysConfigInfo.query.filter(
            *query_filter).all()
        #借助marshmallow 格式化数据
        result_list_dict = sys_config_info_schema.dump(result,many=True)
        logger.info("查询配置项，响应体是{}".format(result_list_dict))
    except Exception as e:
        logger.error(e)
        return False, e
