...
def sync_sub_domain_info(account_username):
    ...
    if status_domain and status_token:
        for root_domain_item in root_domain:
            pool_arr.append(pool.submit(async_sub_domain_record, current_app._get_current_object(), account_username,
                                        account_token.token, root_domain_item.domain_name))
...
def async_sub_domain_record(self_app, account_username, account_token, root_domain):
    logger.info(
        "进入异步线程去处理用户{},token是{},{}域名的记录信息".format(account_username, account_token, root_domain))
    ...
    # 字典为空判断，为空意味着没有解析
    recordresult = json.loads(res.content.decode('utf-8'))['records'] if bool(
        json.loads(res.content.decode('utf-8'))) else []
    # 组装批量插入数据[{}]
